...
// Generate Base Tool Pages if they don't exist
toolsData.forEach((toolData) => {
  const toolSlug = toolData.tool;
  const toolName = toolSlug.split('-').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' ');
  const pageDir = path.join(baseDir, toolSlug);
...
export const metadata = resolveMetadata(buildProductMeta({
  title: "${toolName} | Free Online Generator",
  description: "Use our free online ${toolName}. Fast, secure, and runs entirely in your browser.",
  slug: "/${toolSlug}",
}));
  ...
  return (
    <div className="container mx-auto py-12 px-4 max-w-4xl">
      ...
      <div className="bg-muted/30 border border-muted-foreground/20 rounded-xl p-8 mb-12 text-center">
        ...
        <p className="mb-6 text-muted-foreground">
          The ${toolName} interface is currently being built. Check back soon!
        </p>
        <Link href="/">
          <Button size="lg" className="font-bold">
            Back to Home
...
`;
    fs.writeFileSync(pagePath, pageContent);
    console.log(`Generated Tool page: ${pagePath}`);
  }
});
